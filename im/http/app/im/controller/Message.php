...
class Message
{
  public function textMsg()
  {
    ...
    foreach ($member as $value) {
      ...
      $face = getShowPhoto($user_state,$sex,USER_ID,50);
  ...
  public function getChatDetails()
  {
    ...
    $data = [];
    $is_action = 0;
    $chat_list_data = ChatList::field('type,top')
    ->where([
      'user_id' => USER_ID,
      'list_id' => $post_data['list_id'],
    ])
    ...
    if(count($member)){
      foreach ($member as $key => $value) {
        ...
        $face = getShowPhoto($user_state_obj,$db_user->sex,$value['user_id'],70);
  ...
  public function groupMember()
  {
    ...
    foreach ($member_data as $key => &$value) {
      ...
      $value['photo'] = getShowPhoto($state_data,$user_data->sex,$value['user_id'],70);
    }
    $return_data['err'] = 0;
    $return_data['msg'] = 'success';
  ...
  public function getGroupAdmin()
  {
    ...
    if(count($db_data)){
      foreach ($db_data as $value) {
        ...
        $user_state_obj = UserState::field('photo')->where('user_id',$value['user_id'])->find();
        $face = getShowPhoto($user_state_obj,$User->sex,$value['user_id'],70);
