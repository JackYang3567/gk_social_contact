...
class Get
{
	...
	public function base()
	{
		...
		$photo = getShowPhoto($user_state_obj,$user->sex,USER_ID,90);
	...
	{
		$db_data = ChatList::field('list_id,user_ids,no_reader_num,type,top')->where([
			'user_id' => USER_ID,
			'status' => 0,
		])->select()->toArray();
		$data = [];
		if(count($db_data)){
			foreach ($db_data as $key => $value) {
				switch ($value['type']) {
					case 0:
						...
						$photo_path = getShowPhoto($user_state,$db_user->sex,$friend_id,90);
						break;
					case 1:
						/** 群聊 */
			...
			/** 根据消息最后时间排序 */
			$is_field = array_column($data,'time');
			array_multisort($is_field,SORT_DESC,$data);
			/** 再根据消息置顶排序 */
			//$is_field = array_column($data,'top');
			//array_multisort($is_field,SORT_DESC,$data);
		}
		$return_data = [
			'err' => 0,
		...
		if(!isset($post_data['friend']) || $post_data['friend']){
			...
			if(count($db_data)){
				foreach ($db_data as $key => $value) {
					...
					$face = getShowPhoto($user_state_obj,$user->sex,$value['friend_id'],90);
		...
		if($is_show && (count($friend_ids) || isset($post_data['chat']))){
			...
			if(count($db_circle_data)){
				foreach ($db_circle_data as $key => $value) {
					...
					$face = getShowPhoto($user_state_data,$db_user->sex,$value['user_id'],90);
	...
	public function chatData()
	{
		...
		if(count($db_data)){
			...
			foreach ($db_data as $key => $value) {
				...
				$face = getShowPhoto($user_state,$sex,$value['user_id'],70);
	...
	public function searchUser()
	{
			...
			foreach ($data as &$value) {
				...
				$value['photo'] = getShowPhoto($user_state_data,$value['sex'],$value['id'],90);
	...
	public function details()
	{
		...
		$circle = [];
		$phone = '';
		$user_data = User::get($post_data['user_id']);
		if(!$user_data){
			$return_data['msg'] = '没有这个用户';
			return json($return_data);
		}
		if($post_data['user_id'] == USER_ID || count($db_friend_data)){
			$is_friend = 1;
		...
		if(isset($post_data['in']) && $post_data['in'] == 1){
			...
			->select()
			->toArray();
			if(count($friend_apply_data)){
				$content = $friend_apply_data[0]['content'];
				$from = $friend_apply_data[0]['from'];
				$apply_id = $friend_apply_data[0]['id'];
			}
		}
		$return_data['err'] = 0;
		$return_data['msg'] = 'success';
		$user_state_obj = UserState::field('photo')->where('user_id',$post_data['user_id'] * 1)->find();
		$photo = getShowPhoto($user_state_obj,$user_data->sex,$post_data['user_id'],90);
		$return_data['data'] = [
			...
			'show_friend' => [
			 ...
			 'from' => ['搜索登陆名添加','搜索手机号码添加','搜索邮箱添加','扫码添加'][$from],
			 'content' => $content,
			 'sex' => $user_data->sex,
			 'apply_id' => $apply_id,
		];
  ...
  public function index()
  {
	...
	public function applyFriend()
	{
		...
		->field('apply_user_id,content,action,time')
		->order('time','DESC')
		->select()
		->toArray();
		if(count($db_data)){
